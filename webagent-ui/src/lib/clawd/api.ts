...
import type { AgentTurnRecord } from "./agent-turns";
...
export async function deleteKnowledgeBase(
  ...
  await request<void>(`/v1/knowledge-bases/${encodeURIComponent(knowledgeBaseId)}`, {
    method: "DELETE",
  }, auth);
}
...
export async function listAgentConversations(
  ...
  return request<AgentConversationRecord[]>("/v1/agent/conversations", undefined, auth);
}
...
export async function createAgentConversation(
  ...
  return request<AgentConversationRecord>("/v1/agent/conversations", {
    method: "POST",
    body: JSON.stringify(payload),
  }, auth);
}
...
export async function listAgentTurns(
  ...
  return request<AgentTurnRecord[]>(
    `/v1/agent/conversations/${encodeURIComponent(conversationId)}/turns`,
    undefined,
    auth,
  );
}
...
export async function interruptAgentConversation(
  ...
  const data = await request<{ conversation: AgentConversationRecord }>(
    `/v1/agent/conversations/${encodeURIComponent(conversationId)}/interrupt`,
    {
      method: "POST",
    },
    auth,
...
export async function createExpertPanelRun(
  threadId: string,
  payload: CreateExpertPanelRunRequest,
  auth: RequestAuth,
): Promise<ExpertPanelRunResponse> {
  return request<ExpertPanelRunResponse>(
    `/v1/threads/${encodeURIComponent(threadId)}/expert-panel-runs`,
    {
      method: "POST",
      body: JSON.stringify(payload),
    },
...
export async function getExpertPanelRun(
  threadId: string,
  runId: string,
  auth: RequestAuth,
): Promise<ExpertPanelRunResponse> {
  return request<ExpertPanelRunResponse>(
    `/v1/threads/${encodeURIComponent(threadId)}/expert-panel-runs/${encodeURIComponent(runId)}`,
    undefined,
    auth,
  );
}

export function expertPanelRunEventsUrl(
  threadId: string,
  runId: string,
  auth: RequestAuth,
): string {
  return `${API_BASE}${threadPath(
    `/v1/threads/${encodeURIComponent(threadId)}/expert-panel-runs/${encodeURIComponent(runId)}/events`,
    auth,
  )}`;
}
...
export async function deleteThread(threadId: string, auth: RequestAuth): Promise<void> {
  await request<void>(`/v1/threads/${encodeURIComponent(threadId)}`, {
    method: "DELETE",
  }, auth);
}
