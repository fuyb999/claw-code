...
export type ExpertPanelRunStatus = "queued" | "running" | "succeeded" | "failed";
export type ExpertPanelRunExpertStatus = "queued" | "running" | "retrying" | "succeeded" | "failed";

export interface ExpertPanelRunExpert {
  skill: string;
  scope: SkillScope;
  label: string;
  description?: string | null;
  status: ExpertPanelRunExpertStatus;
  attempts: number;
  content?: string | null;
  citations: string[];
...
export interface ExpertPanelRunResponse {
  run_id: string;
  thread_id: string;
  status: ExpertPanelRunStatus;
  retry_count: number;
  concurrency_limit: number;
  experts: ExpertPanelRunExpert[];
}

export interface ExpertPanelRunEventEnvelope {
  kind: string;
  at_ms: number;
  payload: {
    run_id?: string;
    ...
    expert?: string;
    attempt?: number;
    status?: ExpertPanelRunStatus | ExpertPanelRunExpertStatus;
    [key: string]: unknown;
  };
}
